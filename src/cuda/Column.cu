 ...
 */
__device__ void initColumn(Column* col, Region* region, int srcPosX, int srcPosY,
    int posX, int posY) {
  col->region = region;
  col->numCells = region->cellsPerCol;
  col->cells = (Cell *) malloc(col->numCells * sizeof(Cell));
  int i;
  ...
  col->proximalSegment = (Segment *) malloc(sizeof(Segment));
 ...
 */
__device__ void deleteColumn(Column* col) {
  int i;
  for(i=0; i<col->numCells; ++i)
 ...
 */
__device__ void nextColumnTimeStep(Column* col) {
  int i;
  for(i=0; i<col->numCells; ++i)
 ...
 */
__device__ float getOverlapPercentage(Column* col) {
 ...
 */
__device__ Cell* getBestMatchingCell(Column* col, Segment** bestSegPtr, int* segmentID,
 ...
 */
__device__ void computeOverlap(Column* col) {
 ...
 */
__device__ void updateColumnPermanences(Column* col) {
 ...
 */
__device__ void increasePermanences(Column* col, int amount) {
  int i;
  Segment* seg = col->proximalSegment;
  for(i=0; i<seg->numSynapses; ++i) {
 ...
 * after inhibition.
 */
__device__ void updateActiveDutyCycle(Column* col) {
 ...
 * St = a * Yt + (1-a)*St-1
 */
__device__ void updateOverlapDutyCycle(Column* col) {
 ...
 */
__device__ float boostFunction(Column* col, float minDutyCycle) {
 ...
 */
__device__ void performBoosting(Column* col) {
