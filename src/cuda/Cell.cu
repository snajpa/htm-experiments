 ...
 */
__device__ void initCell(Cell* cell, Column* column, int index) {
  cell->column = column;
  cell->index = index;
  cell->isActive = false;
  ...
  cell->segments = (Segment *) malloc(cell->allocatedSegments * sizeof(Segment));

  cell->numSegUpdates = 0;
  cell->allocatedSegUpdates = 5;
  cell->segmentUpdates = (SegmentUpdateInfo *) malloc(cell->allocatedSegUpdates * sizeof(SegmentUpdateInfo));
 ...
 */
__device__ void initInputCell(Cell* cell, Region* region, int index) {
  cell->region = region;
  cell->column = NULL;
  cell->index = index;
...
__device__ Segment* createCellSegment(Cell* cell) {
  ...
  if(cell->numSegments == cell->allocatedSegments) {
    ...
    cell->segments = (Segment *) malloc(newAllocation * sizeof(Segment));
 ...
 */
__device__ SegmentUpdateInfo* updateSegmentActiveSynapses(Cell* cell, bool previous,
  ...
  if(cell->numSegUpdates == cell->allocatedSegUpdates) {
    ...
    cell->segmentUpdates = (SegmentUpdateInfo *)
        malloc(newAllocation * sizeof(SegmentUpdateInfo));
 ...
 */
__device__ Segment* getBestMatchingSegment(Cell* cell, int numPredictionSteps, bool previous,
    int* segmentID) {
  Segment* bestSegment = NULL;
