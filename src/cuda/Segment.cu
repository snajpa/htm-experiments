 ...
 */
__device__ void initSegment(Segment* seg, int segActiveThreshold) {
  seg->numSynapses = 0;
  seg->allocatedSynapses = 10;
  seg->synapses = (Synapse *)malloc(seg->allocatedSynapses * sizeof(Synapse));
 ...
 */
__device__ void deleteSegment(Segment* seg) {
  free(seg->synapses);
  seg->synapses = NULL;
  seg->numSynapses = 0;
}
 ...
 * can be determined.
 */
__device__ void nextSegmentTimeStep(Segment* seg) {
 ...
 */
__device__ void processSegment(Segment* seg) {
 ...
 */
__device__ void setNumPredictionSteps(Segment* seg, int steps) {
  if(steps < 1) steps = 1;
 ...
 */
__device__ Synapse* createSynapse(Segment* seg, Cell* inputSource, int initPerm) {
 ...
 * this segment.
 */
__device__ void updateSegmentPermanences(Segment* seg, bool increase) {
  int i;
  for(i=0; i<seg->numSynapses; ++i) {
    Synapse* syn = &(seg->synapses[i]);
    if(increase)
 ...
 */
__device__ bool wasSegmentActiveFromLearning(Segment* seg) {
  int i, c=0;
  for(i=0; i<seg->numSynapses; ++i) {
