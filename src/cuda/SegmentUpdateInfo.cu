 ...
 */
__device__ void randomSample(Cell** cells, int n, Cell** ssCells, int m) {
  Cell** ss = ssCells;
  int i, j, k;
 ...
 */
__device__ void initSegmentUpdateInfo(SegmentUpdateInfo* info, Cell* cell, int segmentID,
  ...
  if(segment!=NULL) {
    if(previous) {
      ...
      info->activeSynapseIDs = (int *) malloc(info->numActiveSyns * sizeof(int));
      ...
      info->activeSynapseIDs = (int *) malloc(segment->numActiveConnectedSyns * sizeof(int));
      for(i=0, s=0; i<segment->numSynapses; ++i) {
  ...
  Cell** learningCells = (Cell**) malloc(allocatedLearnCells * sizeof(Cell*));
    ...
    for(x=minX; x<=maxX; ++x) {
      for(y=minY; y<=maxY; ++y) {
		...
		free(learningCells);
                ...
                learningCells = (Cell **) malloc(newAllocation * sizeof(Cell*));
  ...
  if(numLearnCells > 0 && synCount > 0) {
    Cell** ss = (Cell **) malloc(synCount * sizeof(Cell*));
 ...
 */
__device__ void deleteSegmentUpdateInfo(SegmentUpdateInfo* info) {
 ...
 */
__device__ void createSynapsesToLearningCells(SegmentUpdateInfo* info, Segment* seg) {
 ...
 */
__device__ Segment* createCellSegmentFromInfo(SegmentUpdateInfo* info) {
 ...
 */
__device__ void updateInfoPermanences(SegmentUpdateInfo* info) {
 ...
 */
__device__ void decreaseInfoPermanences(SegmentUpdateInfo* info) {
  Segment* segment = &(info->cell->segments[info->segmentID]);
  int i;
  for(i=0; i<info->numActiveSyns; ++i) {
    Synapse* syn = &(segment->synapses[info->activeSynapseIDs[i]]);
 ...
 */
__device__ void applySegmentUpdates(SegmentUpdateInfo* info, bool positiveReinforcement) {
  Segment* segment = NULL;
  if(info->segmentID >= 0)
