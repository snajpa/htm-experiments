 ...
 */
__device__ float averageReceptiveFieldSize(Region* region) {
  int i,j;
  int n = 0;
  double sum = 0.0;
...
__global__ void newRegionHardcoded(Region *region, int inputSizeX, int inputSizeY, int localityRadius,
  ...
  region->columns = (Column *) malloc(region->numCols * sizeof(Column));
  if(region->columns==NULL) return;
...
__global__ void newRegion(Region *region, int inputSizeX, int inputSizeY, int colGridSizeX, int colGridSizeY,
  ...
  region->inputCells = (Cell *) malloc(nc * sizeof(Cell));
  Cell** ssInputCells = (Cell **) malloc(nc * sizeof(Cell*));
  Cell** randCells = (Cell **) malloc(synapsesPerSegment * sizeof(Cell*));
  for(i=0; i<nc; ++i) {
 ...
 */
__device__ bool isWithinKthScore(Region* region, Column* col, int k) {
